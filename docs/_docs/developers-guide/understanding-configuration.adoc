...
The XML configuration file is a Spring Bean definition file that must
contain the `IgniteConfiguration` bean. When starting a node from the
command line, pass the configuration file as a parameter to the
`ignite.sh|bat` script, as follows:
