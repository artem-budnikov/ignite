...
the node will shut down without waiting for the completion of the rebalancing process.
...
Removing the node from the baseline topology will start the rebalancing process on the remaining nodes.
...
For example, if the number of backups for a cache is set to 1 and you stop 2 nodes, there is a chance that both the primary and backup copy of a partition will become unavailable to the rest of the cluster.
...
The node will check if all the partitions are available to the remaining nodes and only then exit the process.
//This has to do with the number of partition backups configured for caches.
//This property is designed to prevent this situation.
